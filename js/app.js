...
/* Showing all product UI interfase */
...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="single-product fw-bold">
      <div>
    <img class="product-image" id="card-img" src=${image}></img>
      </div>
     <div class="text-start px-3 mx-3">
...
/* value convert: integer to float */
...
/* product price update in cart */

const updatePrice = (id, value) => {
  const convertedOldPrice = getInputValue(id);
  const convertPrice = parseFloat(value);
  const total = convertedOldPrice + convertPrice;
  document.getElementById(id).innerText = total.toFixed(2); /* using toFixed to add 2 decimal */
};
...
/* update delivery charge and total Tax in cart */
...
/* added product price total update */
